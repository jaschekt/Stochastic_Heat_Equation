function backwardsEuler(odefun,M,tSpan,tSteps,u0,tol,process)
%BACKWARDSEULER Solves the discrete initial value problem
%   M ( u_k - u_k-1 )/?t + F( t_k , u_k ) = 0
%   u_0 = u0
%for k = 1, 2, ..., tSteps
%   Input:
...
for k = 1:tSteps
    ...
    % u at step k
    u = newton(@(u) newtonfun(t,u),u0,tol); %there's going to be not much change in each time step, so the previous snapshot u0 should already be very close to u
