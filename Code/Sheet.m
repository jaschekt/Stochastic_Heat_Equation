...
classdef Sheet
   ...
   methods (Static)
       ...
       function STN = computeNoise(sigma)
           BS = Sheet.computeSheet();
           STN = zeros(Sheet.N,Sheet.N);
           for i=1:Sheet.N
               for j=1:Sheet.N
                   for k=1:Sheet.N-1
                        STN(i,j) = STN(i,j)+ sigma*(BS(min(k+1,i),j)-BS(min(k,i),j));
                   end
               end
           end
           figure
           ...
           figure
           surf(linspace(1/Sheet.N,1,Sheet.N),linspace(1/Sheet.N,1,Sheet.N),STN,'edgealpha','0');
       end
       function plotit()
           BS = zeros(Sheet.N,Sheet.N);
